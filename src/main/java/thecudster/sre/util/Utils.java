...
public class Utils {
    ...
    public static String getUnformattedChat(ClientChatReceivedEvent event) {
        return StringUtils.stripControlCodes(event.message.getUnformattedText());
    }
    public static void sendHelp() {
        sendMsg(EnumChatFormatting.GOLD + "✰ SkyblockReinvented Commands & Help\n" +
                ...
                EnumChatFormatting.AQUA + "   • /sre discord: join our discord!" +
                EnumChatFormatting.AQUA + "   • /sre github: view the source code for SRE on GitHub!" +
    ...
    public static boolean containsAnyOf(String check, String[] needsToContain) {
        for (String s : needsToContain) {
            if (check.contains(s)) return true;
        }
        return false;
    }
    public static boolean containsAllOf(String check, String[] needsToContain) {
        for (String s : needsToContain) {
