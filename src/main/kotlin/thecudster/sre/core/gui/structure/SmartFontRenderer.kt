...
class SmartFontRenderer : FontRenderer(
    ...
    fun setFontHeight(newFontHeight: Int) {
        super.FONT_HEIGHT = newFontHeight
    }
    ...
    fun drawRainbowText(input: String, x: Float, y: Float, alignment: TextAlignment, shadow: TextShadow?): Float {
    ...
    private fun drawCritText(input: String, x: Float, y: Float, alignment: TextAlignment, shadow: TextShadow?): Float {
        ...
        GlStateManager.scale(20.0f, 20.0f, 1f)
        posX = x
        posY = y
        for (c in input.toCharArray()) {
            val dif = posX.toLong() * 10 - posY.toLong() * 10
            ...
            val green = (85.0f * Math.cos(time % 2000 * Math.PI / 1000).toFloat() + 170) / 255.0f
    ...
    companion object {
        ...
        private fun decodeCommonColor(character: Char): Int? {
            ...
            if ('a' <= character && character <= 'f') return minecraftColors[character.toInt() + (10 - 'a'.toInt())]
            return if ('A' <= character && character <= 'F') minecraftColors[character.toInt() + (10 - 'A'.toInt())] else null
        }
    }
}